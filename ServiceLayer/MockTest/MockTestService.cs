...
namespace ServiceLayer.MockTest
{
    public class MockTestService : IMockTestService
    {
        ...
        private string BuildAnalysisPrompt(DataLayer.DTOs.UserAnswer.ExamAttemptDetailResponseDTO examAttempt, List<string> articleNames)
        {
            ...
            sb.AppendLine("    \"Điểm mạnh cụ thể 1 (kèm số liệu từ kết quả bài làm, ví dụ: 'Listening: đạt 8/10 câu đúng ở Part 2 - nghe hiểu hội thoại ngắn tốt')\",");
            sb.AppendLine("    \"Điểm mạnh cụ thể 2 (phải dựa vào điểm số thực tế của từng kỹ năng)\",");
            sb.AppendLine("    \"Điểm mạnh cụ thể 3\"");
            sb.AppendLine("  ],");
            sb.AppendLine("  \"Weaknesses\": [");
            sb.AppendLine("    \"Điểm yếu cụ thể 1 (kèm số liệu và phân tích nguyên nhân, ví dụ: 'Reading: chỉ đạt 3/10 câu Part 7 - yếu kỹ năng đọc hiểu đoạn văn dài')\",");
            sb.AppendLine("    \"Điểm yếu cụ thể 2 (phải phân tích sâu dựa vào các câu sai và điểm thấp)\",");
            sb.AppendLine("    \"Điểm yếu cụ thể 3\"");
            sb.AppendLine("  ],");
            ...
            sb.AppendLine("- Strengths: Phải là 3-5 điểm mạnh CỤ THỂ, KÈM THEO SỐ LIỆU từ kết quả bài làm (số câu đúng, điểm số, %). KHÔNG được viết chung chung!");
            sb.AppendLine("  + Ví dụ TỐT: 'Listening Part 1-2: Đạt 18/20 câu đúng (90%) - Rất tốt ở kỹ năng nghe hội thoại ngắn'");
            sb.AppendLine("  + Ví dụ XẤU: 'Nghe tốt' (quá chung chung, không có số liệu)");
            sb.AppendLine("- Weaknesses: Phải là 3-5 điểm yếu CỤ THỂ, KÈM THEO:");
            sb.AppendLine("  + Số liệu cụ thể (số câu sai, điểm thấp ở phần nào)");
            sb.AppendLine("  + Phân tích nguyên nhân (sai nhiều ở dạng câu hỏi nào, kỹ năng gì còn yếu)");
            sb.AppendLine("  + Ví dụ TỐT: 'Reading Part 7: Chỉ đạt 5/15 câu (33%) - Yếu kỹ năng đọc hiểu đoạn văn dài, suy luận thông tin'");
            sb.AppendLine("  + Ví dụ XẤU: 'Đọc yếu' (không có phân tích chi tiết)");
